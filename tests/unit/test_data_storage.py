...
class DataStorageTest(TestCase):
    ...
    def test_simple_nested_entry_exit_calls(self):
        ...
        # First exit testing
        first_exit_query_profiled_data = query_profiler_thread_local_storage.exit_profiler_mode()
        self.assertTrue(query_profiler_thread_local_storage._query_profiler_enabled)

        # Size of list does not decrease, and stack should contain only first enter index
        self.assertEqual(len(query_profiler_thread_local_storage._query_profiled_data_list), 2)
        self.assertListEqual(query_profiler_thread_local_storage._entry_index_stack, [0])
        expected_query_profiled_summary_data = QueryProfiledSummaryData(
            sql_statement_type_counter=Counter({SqlStatement.SELECT: 2}),
            exact_query_duplicates=2,
            total_query_execution_time_in_micros=self.query_execution_time_in_micros * 2,
        ...
        second_exit_query_profiled_data = query_profiler_thread_local_storage.exit_profiler_mode()
    ...
    def test_complex_nested_entry_exit_calls(self):
        ...
        self.assertListEqual(query_profiler_thread_local_storage._entry_index_stack, [0, 1, 2])
        ...
        # Size of list does not decrease, and stack should contain only first enter index
        self.assertEqual(len(query_profiler_thread_local_storage._query_profiled_data_list), 3)
        self.assertListEqual(query_profiler_thread_local_storage._entry_index_stack, [0, 1])
        expected_query_profiled_summary_data = QueryProfiledSummaryData(
        ...
        # Before second exit
        self.assertListEqual(query_profiler_thread_local_storage._entry_index_stack, [0, 1, 3])

        # Second exit
        second_exit_query_profiled_data = query_profiler_thread_local_storage.exit_profiler_mode()
        self.assertTrue(query_profiler_thread_local_storage._query_profiler_enabled)
        # Size of list does not decrease, and stack should contain only first enter index
        self.assertEqual(len(query_profiler_thread_local_storage._query_profiled_data_list), 4)
        self.assertListEqual(query_profiler_thread_local_storage._entry_index_stack, [0, 1])
        expected_query_profiled_summary_data = QueryProfiledSummaryData(
            sql_statement_type_counter=Counter({SqlStatement.SELECT: 1}),
            exact_query_duplicates=0,
        ...
        # Third exit
        third_exit_query_profiled_data = query_profiler_thread_local_storage.exit_profiler_mode()
        self.assertTrue(query_profiler_thread_local_storage._query_profiler_enabled)
        # Size of list does not decrease, and stack should contain only first enter index
        self.assertEqual(len(query_profiler_thread_local_storage._query_profiled_data_list), 4)
        self.assertListEqual(query_profiler_thread_local_storage._entry_index_stack, [0])
        ...
        # Fourth exit
        fourth_exit_query_profiled_data = query_profiler_thread_local_storage.exit_profiler_mode()
        self.assertFalse(query_profiler_thread_local_storage._query_profiler_enabled)
        # Size of list does not decrease, and stack should contain only first enter index
        self.assertEqual(len(query_profiler_thread_local_storage._query_profiled_data_list), 0)
